...
`auto_start` 最初含义是「开机自启」。在修复启动窗口问题时，第一轮修复把它重载成了「总是最小化启动」——这会改变手动双击启动的行为。

**教训：** 配置字段是契约。一个字段表达两个意思 = 行为不可预测。当需要两种行为时，要么用两个字段，要么保持单一语义并通过其他机制（命令行参数、注册表）传递附加信息。

### 让用户确认语义变更

修复时引入了「`auto_start=true` 且手动启动 → 仍隐藏窗口」的行为变化。从代码逻辑看这没错，但与用户直觉冲突：在设置里开的是「开机自启」，为什么双击也自动隐藏？

**教训：** 影响用户可见行为的设计变更必须先问。这个在全局 CLAUDE.md 第 6 条有明确要求，但实现时被忽略了。遵循自己的规则。

### 修链路比加旁路好

启动窗口问题的表现：`auto_start` 已开启但开机后窗口仍弹出。

第一轮修复加了旁路（读 config 兜底），偏离了原有设计：`--minimized` arg → setup 决定窗口。
第二轮回头修了链路本身：加固 `reg.exe` 错误处理 + 日志，让注册表写入可观测、可调试。

**教训：** 遇到链路故障时，旁路（读 config）只是掩盖症状，修链路（加固 reg.exe）才是治本。加旁路时问自己：这条链路为什么断了？修好它比绕开它更难吗？

### `let _ =` 的沉默代价

`set_auto_start` 中 `let _ = reg.exe.output()` 静默丢弃了所有错误。如果 reg.exe 因权限/路径问题失败，没有任何痕迹。

**教训：** 系统命令（`reg.exe`、`fs` 操作等）的返回值不应该静默丢弃。`let _ =` 只适合「失败了也无关紧要」的场景，不适合「负责建立开机自启」这种核心路径。至少加个日志。

### 2026-05-24 复盘：auto-start 全迭代

```
round 1: 移除死字段 minimize_to_tray           ✅ 简化配置
round 2: 加 config.auto_start 兜底             ❌ 语义重载
round 3: 回退到 --minimized arg + 加固 reg.exe  ✅ 修链路
```

**核心经验：** 三个 round 的核心区别不是技术方案，而是**对问题本质的理解**：
- Round 2 认为问题是「arg 传不过来，加个 config 兜底」
- Round 3 认识到问题是「reg.exe 可能失败且没有任何日志，先让它可观测」

如果一开始就给 `set_auto_start` 加了错误处理，round 2 可能根本不需要。
