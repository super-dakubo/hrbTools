  ...
  └── 游戏名/
      └── 存档位名/
          └── 2026-05-07 14-30-22 1/     ← 时间戳 + 空格 + 描述（默认序号）
              ├── meta.json    ← { "original_file_path": "...", "display_name": "...", "description": "1" }
              └── save.dat
```
...
9. **保存存档** → 备份文件夹名为 `时间戳 + 空格 + 序号`（如 `2026-05-07 14-30-22 1`），描述（序号）默认从 1 开始递增，每个存档位独立计数；可改名；若未设置备份根目录则弹提示引导去设置
10. **重命名备份** → 只允许修改描述部分（时间戳不可改）
11. **打开设置** → 点击顶部栏齿轮 ⚙，弹出模态设置面板，可设置备份根目录
12. **关闭设置** → 点击 × 或背景遮罩，不破坏当前面板状态
