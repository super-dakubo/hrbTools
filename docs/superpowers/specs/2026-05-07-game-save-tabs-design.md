...
struct SlotConfig {
    name: String,
    next_backup_number: u32,        // 默认 1，创建备份后自增
    key_file_patterns: Vec<String>, // 默认空（空 = 所有文件都哈希）
}
...
备份根目录/
  └── 游戏名/
      └── 存档位名/
          └── 2026-05-07 14-30-22 1/     ← 时间戳 + 空格 + 描述（默认序号）
              ├── meta.json    ← { "original_file_path": "...", "display_name": "...", "description": "1", "content_hash": "abc123..." }
              └── save.dat
```

- 文件夹名格式：`YYYY-MM-DD HH-MM-SS 描述`
- 描述默认为递增序号（1, 2, 3...），每个存档位独立计数
- 改名只改描述部分，时间戳不可改
- `display_name` 在 meta.json 中同此格式
...
**恢复时的保护**：
1. 计算当前原文件的哈希
2. 在备份链中查找匹配的 `content_hash`
3. 有匹配 → 当前文件已有备份，直接覆盖恢复
4. 无匹配 → 弹窗"当前存档未备份，是否需要先备份再恢复？"
   - 是 → 先自动备份当前文件，再执行恢复
   - 否 → 直接覆盖（跳过保护）
...
5. **新增游戏** → 点击末尾 `+`，自动命名为 `游戏N`（N 为当前数量+1），新增后可直接内联改名
6. **重命名游戏** → 双击游戏标签文字进入编辑，回车确认
7. **新增存档位** → 点击末尾 `+`，自动命名为 `存档N`（N 为当前数量+1），新增后可直接内联改名
8. **重命名存档位** → 双击胶囊文字进入编辑，回车确认
9. **配置关键文件** → 每个存档位可设置 glob 模式（如 `*.dat`, `save_*.*`），仅匹配文件参与哈希；默认空 = 所有文件
10. **保存存档** → 备份文件夹名为 `时间戳 + 空格 + 序号`，计算哈希写入 meta.json；保存前检查哈希去重，相同则提示跳过；若未设置备份根目录则弹提示引导去设置
11. **重命名备份** → 只允许修改描述部分（时间戳不可改）
